...
namespace PatchKit.UnityEditorExtension.Core {
    public static class AppBuild {
        ...
        [NotNull] private static readonly string[] WindowsPdbFiles = {
            "player_win_x86.pdb",
        ...
        public static string Location {
            get {
                string value = EditorUserBuildSettings.GetBuildLocation(EditorUserBuildSettings.activeBuildTarget);
        ...
        private static bool AreThereOnlyBuildEntries([NotNull] string location, params string[] buildFiles) {
            if (location == null) {
            ...
            switch (Platform) {
                ...
                case AppPlatform.Windows64:
                    if (!location.EndsWith(".exe")) {
                        return "Invalid build location file extension. Should be .exe.";
                    }
                    
                    /* This part check if the .exe file selected is in the good directory. The problem is in the newest version of unity there are new files or folder which disable the possibility of selected a build folder.
                    string winBuildFileName = Path.GetFileName(location);
                    string winBuildDirName = winBuildFileName.Replace(".exe", "_Data");

                    if (!AreThereOnlyBuildEntries(location, winBuildFileName, winBuildDirName, "MonoBleedingEdge", "Mono", "UnityCrashHandler32.exe", "UnityCrashHandler64.exe", "UnityPlayer.dll", "WinPixEventRuntime.dll")) {
                ...
                case AppPlatform.Mac64:
                    if (!location.EndsWith(".app")) {
                        return "Invalid build location file extension. Should be .app.";
                    }
        ...
        public static void OpenLocationDialog() {
            ...
            do {
                ...
                string location = EditorUtility.SaveFilePanel("Select build location:", "", "", extension);
